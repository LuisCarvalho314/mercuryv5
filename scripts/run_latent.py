...
from mercury.latent.state import (LatentState, latent_step,
                                    init_latent_state,
...
def run_latent(level, latent_cfg: LatentParams = None, memory_length = 10):
    # ----- data -----
    ...
    for t, (observation, action, collision) in enumerate(iter_sequence(obs, act,
        ...
        n_latent_states.append(int(latent.g.n))
    ...
    np.savetxt(f"n_latent{level}.csv", n_latent_states, delimiter=",")